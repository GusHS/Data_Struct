...
public class ImagenGUI extends JFrame implements ActionListener, ListSelectionListener
{
    ...
    private byte streamImagen[], streamSong[];
    private OutputStream archivoImagen, archivoSong;
    private InputStream bufferImagen, songInput;
	...
	public void valueChanged(ListSelectionEvent lse){
        if(lse.getValueIsAdjusting()){
            if(lse.getSource()==listaSongs){
				...
				panelPlayStop.add(new JLabel("Selected Song: "+cancionElegida));
        		panelPlayStop.add(bPlay);
				panelPlayStop.add(bStop);
				panelPlayStop.setVisible(true);
				
				socket = conexion.establecerConexion();
				conexion.enviarDatos("sendSong");
				conexion.enviarDatos(cancionElegida);
				try {
					songInput =socket.getInputStream();
				} catch (Exception e) {
    ...
    public void actionPerformed(ActionEvent e)
    {
    	...
    	if(e.getSource()== bArtista)
    	{
			 ...
			 System.out.println("[ GUI ] Artist: "+artista);
 			if(artista.isEmpty())
 			{
