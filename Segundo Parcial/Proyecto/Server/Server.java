...
public class Server extends JFrame
{
	...
	private void enviarWAV(String songName){
		byte streamSong[]= new byte[1024];
		int nb = 0;
		System.out.println("[Server] Song name: "+songName);
		try{
			inputStream = new FileInputStream("Songs/"+songName+".wav");
			bufferSong = socket.getOutputStream();

			while ((nb=inputStream.read(streamSong)) > 0) {
				bufferSong.write(streamSong,0,nb);
			}
		}catch(IOException ioe){
			System.out.println("Error: "+ioe);
		}
		...
		try
		{
			...
			while(true)
			{
                ...
                if(transaccion.equals("obtenerSongs"))
                {
                	...
                	int numSongs = listaSongs.length;
                	enviarDatos(""+numSongs);
                	
                	int i = 0;
                	while(i<numSongs)
                	{
                		enviarDatos(listaSongs[i]);
                		i++;
                	}
				}
				
				if (transaccion.equals("sendSong")){
					String songName = recibirDatos();
